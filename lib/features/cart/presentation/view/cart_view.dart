...
class _CartViewState extends State<CartView> {
  List<CartEntity> _localCartItems = [];
      ...
      child: BlocBuilder<CartBloc, CartState>(
        builder: (context, state) {
          if (state is CartLoading) {
            ...
            if (!_initialized) {
              _localCartItems = List<CartEntity>.from(state.items);
              _initialized = true;
            }
...
class CartCard extends StatefulWidget {
  ...
  final String size;
  final int quantity;
  final VoidCallback onStaticRemove;
          ...
          children: [
            ClipRRect(
              ...
              child: CachedNetworkImage(
                imageUrl: "${ApiEndpoints.serverAddress}/${widget.image}",
                width: 80,
                height: 80,
                fit: BoxFit.cover,
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        child: Text('Color: ${widget.color}', style: const TextStyle(fontSize: 12)),
                      ),
                      ...
                      Container(
                        ...
                        child: Text('Size: ${widget.size}', style: const TextStyle(fontSize: 12)),
                      ),
                    ],
                  ),
                  const SizedBox(height: 8),
                  Text('Rs. ${widget.price.toStringAsFixed(2)}', style: const TextStyle(color: Colors.deepOrange, fontWeight: FontWeight.bold)),
                ],
              ),
            ),
            ConstrainedBox(
